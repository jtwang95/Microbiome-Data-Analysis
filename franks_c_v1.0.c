...
double CalculateFValue(double **Mat,int i,double* Product,int *GroupIndex,int GroupNum,int n);
...
int RankFValue(double **Mat, char **GroupInfo,char **OTUName,int n,int p){
	...
	double otus_product[n];
	double product;
	for(int j=0;j<n;j++){
		product = 1.0;
		for(int i=0;i<p;i++){
			product*=pow(Mat[i][j],1.0/p);	
		}
		otus_product[j] = product;
	}
	for(int i=0;i<p;i++){
		fvalue[i]=CalculateFValue(Mat,i,otus_product,GroupIndex,GroupNum,n);
	}
	//check GroupNum
	//replace f value (inf & nan)
	QuickSort(fvalue,0,p-1,OTUName);
	free(fvalue);
	return 0;
}
...
double CalculateFValue(double **Mat,int i,double *Product,int *GroupIndex,int GroupNum,int n){
	...
	for(int a=0;a<n;a++){
		*p[GroupIndex[a]] = log(Mat[i][a]/Product[a]);
		p[GroupIndex[a]]++;
	}
